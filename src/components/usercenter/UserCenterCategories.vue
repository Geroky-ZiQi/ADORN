<template>
 
  <div>
    <section class="browse_categories_sec">
    	<div class="container">
    		<div class="browse_categories">
    			<div class="row">
    				<div class="col-lg-2 col-md-4 col-sm-4 col-6 full_wdth">
    					<div class="br-channel">
    						<div class="br-channel-img">
    							<a href="#" title="">
    								<img src="../../assets/images/resources/br1.jpg" alt="">
    							</a>
    						</div>
    						<div class="br-info">
    							<h3><a href="#" title="">减肥塑形</a></h3>
    							<span>230 相关笔记</span>
    						</div>
    					</div><!--br-channel end-->
    				</div>
    				<div class="col-lg-2 col-md-4 col-sm-4 col-6 full_wdth">
    					<div class="br-channel">
    						<div class="br-channel-img">
    							<a href="#" title="">
    								<img src="../../assets/images/resources/br2.jpg" alt="">
    							</a>
    						</div>
    						<div class="br-info">
    							<h3><a href="#" title="">美容护肤</a></h3>
    							<span>191 相关笔记</span>
    						</div>
    					</div><!--br-channel end-->
    				</div>
    				
    				<div class="col-lg-2 col-md-4 col-sm-4 col-6 full_wdth">
    					<div class="br-channel">
    						<div class="br-channel-img">
    							<a href="#" title="">
    								<img src="../../assets/images/resources/br4.jpg" alt="">
    							</a>
    						</div>
    						<div class="br-info">
    							<h3><a href="#" title="">养生专栏</a> <span class="verify_ic"><i class="icon-tick"></i></span></h3>
    							<span>48 相关笔记</span>
    						</div>
    					</div><!--br-channel end-->
    				</div>
    				<div class="col-lg-2 col-md-4 col-sm-4 col-6 full_wdth">
    					<div class="br-channel">
    						<div class="br-channel-img">
    							<a href="#" title="">
    								<img src="../../assets/images/resources/br5.jpg" alt="">
    							</a>
    						</div>
    						<div class="br-info">
    							<h3><a href="#" title="">内外调理</a></h3>
    							<span>954 相关笔记</span>
    						</div>
    					</div><!--br-channel end-->
    				</div>
    				<div class="col-lg-2 col-md-4 col-sm-4 col-6 full_wdth">
    					<div class="br-channel">
    						<div class="br-channel-img">
    							<a href="#" title="">
    								<img src="../../assets/images/resources/br6.jpg" alt="">
    							</a>
    						</div>
    						<div class="br-info">
    							<h3><a href="#" title="">底妆部分</a></h3>
    							<span>130 相关笔记</span>
    						</div>
    					</div><!--br-channel end-->
    				</div>
    				
    				<div class="col-lg-2 col-md-4 col-sm-4 col-6 full_wdth">
    					<div class="br-channel">
    						<div class="br-channel-img">
    							<a href="#" title="">
    								<img src="../../assets/images/resources/br8.jpg" alt="">
    							</a>
    						</div>
    						<div class="br-info">
    							<h3><a href="#" title="">眼妆部分</a></h3>
    							<span>191 相关笔记</span>
    						</div>
    					</div><!--br-channel end-->
    				</div>
    				
    				<div class="col-lg-2 col-md-4 col-sm-4 col-6 full_wdth">
    					<div class="br-channel">
    						<div class="br-channel-img">
    							<a href="#" title="">
    								<img src="../../assets/images/resources/br11.jpg" alt="">
    							</a>
    						</div>
    						<div class="br-info">
    							<h3><a href="#" title="">唇妆部分</a></h3>
    							<span>954 相关笔记</span>
    						</div>
    					</div><!--br-channel end-->
    				</div>
    				<div class="col-lg-2 col-md-4 col-sm-4 col-6 full_wdth">
    					<div class="br-channel">
    						<div class="br-channel-img">
    							<a href="#" title="">
    								<img src="../../assets/images/resources/br12.jpg" alt="">
    							</a>
    						</div>
    						<div class="br-info">
    							<h3><a href="#" title="">热门产品排行</a></h3>
    							<span>130 Videos</span>
    						</div>
    					</div><!--br-channel end-->
    				</div>
    				<div class="col-lg-2 col-md-4 col-sm-4 col-6 full_wdth">
    					<div class="br-channel">
    						<div class="br-channel-img">
    							<a href="#" title="">
    								<img src="../../assets/images/resources/br13.jpg" alt="">
    							</a>
    						</div>
    						<div class="br-info">
    							<h3><a href="#" title="">发现新产品</a></h3>
    							<span>230 Videos</span>
    						</div>
    					</div><!--br-channel end-->
    				</div>
    				<div class="col-lg-2 col-md-4 col-sm-4 col-6 full_wdth">
    					<div class="br-channel">
    						<div class="br-channel-img">
    							<a href="#" title="">
    								<img src="../../assets/images/resources/br14.jpg" alt="">
    							</a>
    						</div>
    						<div class="br-info">
    							<h3><a href="#" title="">香水部分</a></h3>
    							<span>191 Videos</span>
    						</div>
    					</div><!--br-channel end-->
    				</div>
    				<div class="col-lg-2 col-md-4 col-sm-4 col-6 full_wdth">
    					<div class="br-channel">
    						<div class="br-channel-img">
    							<a href="#" title="">
    								<img src="../../assets/images/resources/br15.jpg" alt="">
    							</a>
    						</div>
    						<div class="br-info">
    							<h3><a href="#" title="">水乳部分</a> <span class="verify_ic"><i class="icon-tick"></i></span></h3>
    							<span>12 Videos</span>
    						</div>
    					</div><!--br-channel end-->
    				</div>
    				<div class="col-lg-2 col-md-4 col-sm-4 col-6 full_wdth">
    					<div class="br-channel">
    						<div class="br-channel-img">
    							<a href="#" title="">
    								<img src="../../assets/images/resources/br16.jpg" alt="">
    							</a>
    						</div>
    						<div class="br-info">
    							<h3><a href="#" title="">精华面霜类</a> <span class="verify_ic"><i class="icon-tick"></i></span></h3>
    							<span>48 Videos</span>
    						</div>
    					</div><!--br-channel end-->
    				</div>
    			</div>
    		</div><!--</div> end-->
    		<div class="browse_cat_lists">
    			<h3>浏览分类</h3>
    			<ul>
    				<li><a href="#" title="">化妆技巧</a></li>
    				<li><a href="#" title="">底妆部分</a></li>
    				<li><a href="#" title="">眼妆部分</a></li>
    				<li><a href="#" title="">唇妆部分</a></li>
    				<li><a href="#" title="">香水</a></li>
    				<li><a href="#" title="">水乳</a></li>
    				<li><a href="#" title="">精华面霜</a></li>
    			</ul>
    			<ul>
    				<li><a href="#" title="">口红试色</a></li>
    				<li><a href="#" title="">在线测评</a></li>
    				<li><a href="#" title="">油皮专区</a></li>
    				<li><a href="#" title="">敏感肌福利</a></li>
    			</ul>
    			<ul>
    				<li><a href="#" title="">干皮专属</a></li>
    				<li><a href="#" title="">粉底全线测评</a></li>
    				<li><a href="#" title="">小样分装</a></li>
    				<li><a href="#" title="">热门博主</a></li>
    			</ul>
    			<ul>
    				<li><a href="#" title="">眼霜</a></li>
    				<li><a href="#" title="">修容</a></li>
    				<li><a href="#" title="">高光o</a></li>
    				<li><a href="#" title="">腮红</a></li>
    				<li><a href="#" title="">热门测评笔记</a></li>
    				<li><a href="#" title="">不脱妆</a></li>
    			</ul>
    			<div class="clearfix"></div>
    		</div><!--browse_cat_lists end-->
    	</div>
    </section><!--browse-sec end-->
    
    <section class="more_items_sec text-center">
    	<div class="container">
    		<a href="#" title="">
    			<svg width="19" height="24" viewBox="0 0 19 24" fill="none" xmlns="http://www.w3.org/2000/svg">
    				<path d="M18.1618 24.0001H0.838235C0.374412 24.0001 0 23.6261 0 23.1628V5.86052C0 5.39727 0.374412 5.02332 0.838235 5.02332H18.1618C18.6256 5.02332 19 5.39727 19 5.86052V23.1628C19 23.6261 18.6256 24.0001 18.1618 24.0001ZM1.67647 22.3256H17.3235V6.69773H1.67647V22.3256Z" fill="#9494A0"/>
    				<g opacity="0.25">
    				<path opacity="0.25" d="M13.1324 4.18605C12.6685 4.18605 12.2941 3.81209 12.2941 3.34884V1.67442H6.70589V3.34884C6.70589 3.81209 6.33148 4.18605 5.86765 4.18605C5.40383 4.18605 5.02942 3.81209 5.02942 3.34884V0.83721C5.02942 0.373954 5.40383 0 5.86765 0H13.1324C13.5962 0 13.9706 0.373954 13.9706 0.83721V3.34884C13.9706 3.81209 13.5962 4.18605 13.1324 4.18605Z" fill="#9494A0"/>
    				</g>
    				<path d="M9.50001 19.3479C9.28487 19.3479 9.06972 19.267 8.90766 19.1024L5.92634 16.1275C5.59942 15.801 5.59942 15.2707 5.92634 14.9442C6.25325 14.6177 6.78413 14.6177 7.11104 14.9442L9.50001 17.3275L11.8862 14.9442C12.2131 14.6177 12.744 14.6177 13.0709 14.9442C13.3978 15.2707 13.3978 15.801 13.0709 16.1275L10.0924 19.1024C9.93031 19.267 9.71516 19.3479 9.50001 19.3479Z" fill="#9494A0"/>
    				<path d="M9.49999 18.4186C9.03617 18.4186 8.66176 18.0447 8.66176 17.5814V10.3256C8.66176 9.86236 9.03617 9.4884 9.49999 9.4884C9.96382 9.4884 10.3382 9.86236 10.3382 10.3256V17.5814C10.3382 18.0447 9.96382 18.4186 9.49999 18.4186Z" fill="#9494A0"/>
    				<g opacity="0.5">
    				<path opacity="0.5" d="M15.6471 6.69764C15.1832 6.69764 14.8088 6.32369 14.8088 5.86043V4.18601H4.19118V5.86043C4.19118 6.32369 3.81677 6.69764 3.35294 6.69764C2.88912 6.69764 2.51471 6.32369 2.51471 5.86043V3.34881C2.51471 2.88555 2.88912 2.5116 3.35294 2.5116H15.6471C16.1109 2.5116 16.4853 2.88555 16.4853 3.34881V5.86043C16.4853 6.32369 16.1109 6.69764 15.6471 6.69764Z" fill="#9494A0"/>
    				</g>
    			</svg>
    		</a>
    	</div>
    </section><!--more_items_sec end-->			
  </div>
</template>

<script>
</script>

<style>
  /*导入当前页面需要的css文件*/
  @import url("../../assets/css/animate.css");
  @import url("../../assets/css/bootstrap.min.css");
  @import url("../../assets/css/fontello.css");
  @import url("../../assets/css/fontello-codes.css");
  @import url("../../assets/css/style.css");
  @import url("../../assets/css/responsive.css");
  @import url("../../assets/css/color.css");
   
</style>
